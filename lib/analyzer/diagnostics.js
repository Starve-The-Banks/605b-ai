...
export function reduceStatus({ findings, reviewOnly, suspectedUncertain, extracted, classified = [] }) {
  ...
  const hasInformationalInquiry = findings.some(
    (f) => f.category === FINDING_CATEGORIES_V1.INFORMATIONAL && f.type === 'inquiry',
  );
  const isInquiryOnlyReport =
    hasInformationalInquiry &&
    extracted.accounts.length === 0 &&
    extracted.collections.length === 0 &&
    extracted.bankingItems.length === 0 &&
    extracted.publicRecords.length === 0;
  ...
  const hasAnyOutput = findings.length > 0 || reviewOnly.length > 0 || suspectedUncertain.length > 0;

  if (hasHighPriority || (!hasAnyOutput && hasClassifiedHighSeverity)) {
    return { reportStatus: REPORT_STATUS_V1.HIGH_PRIORITY_ISSUE, suspectedUncertain: false };
  }
  if (hasPotential || (!hasAnyOutput && hasClassifiedActionable)) {
    return { reportStatus: REPORT_STATUS_V1.POTENTIAL_ISSUE, suspectedUncertain: false };
  }
  if (
    isInquiryOnlyReport &&
    (extracted.reportType === 'credit_bureau' || extracted.reportType === 'chexsystems')
  ) {
    return { reportStatus: REPORT_STATUS_V1.REVIEW_ONLY, suspectedUncertain: false };
  }
  ...
  if (hasNegativeMarker && hasAnyOutput) {
...
export function buildDiagnostics({
  ...
  const collectionLikeMarkerCount = countCollectionLikeMarkers(extracted);
  const emittedByCategory = summarizeByCategory(findings, reviewOnly, suspectedUncertain);
  const emittedByIssueType = summarizeByIssueType(findings, reviewOnly, suspectedUncertain);
...
function countCollectionLikeMarkers(extracted) {
  const RE = /\b(collections?|collection account|collection agency|debt buyer|assigned to collection|placed for collection|charge[-\s]?off sold|collection\/chargeoff|charge[_-\s]?off|charged\s*off)\b/i;
  const buckets = [...(extracted.accounts || []), ...(extracted.collections || [])];
...
function summarizeByCategory(findings, reviewOnly, suspectedUncertain) {
  const all = [...findings, ...reviewOnly, ...suspectedUncertain];
  return all.reduce((acc, finding) => {
...
function summarizeByIssueType(findings, reviewOnly, suspectedUncertain) {
  const all = [...findings, ...reviewOnly, ...suspectedUncertain];
  return all.reduce((acc, finding) => {
    const key = finding.issueType || 'unknown';
    acc[key] = (acc[key] || 0) + 1;
    return acc;
  }, {});
}
