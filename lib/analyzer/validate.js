...
import { getOperationalTemplate, containsForbiddenWord } from './operationalTemplates.js';
...
export function buildValidatedFindings({ classified, annotations }) {
  ...
  for (const item of classified) {
    ...
    if (isOperational) {
      ...
      if (tmpl) {
        // Validate LLM-authored contextLine; strip if non-compliant or too long.
        const llmContext = annotation?.contextLine;
        const llmContextOk =
          typeof llmContext === 'string' &&
          llmContext.trim().length > 0 &&
          llmContext.length <= 200 &&
          !containsForbiddenWord(llmContext);
        const contextLine = llmContextOk ? llmContext.trim() : tmpl.defaultContextLine;
        if (!llmContextOk && llmContext) {
          suppressions.push({
            itemId: item.itemId,
            stage: 'validate',
            reason: 'operational_context_line_rejected',
          });
        }
