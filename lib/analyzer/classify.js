...
function withClassification(item, classification, rule, reason, confidence) {
  return {
    ...item,
    classification,
    classifierRule: rule,
...
function classifyAccount(item) {
  ...
  if (/charge[-\s]?off|charged\s*off/.test(`${status} ${paymentStatus}`)) {
    return withClassification(item, CLASSIFICATIONS.HIGH_PRIORITY, 'account.charge_off',
      'Status or payment status indicates a charge-off.', 0.95);
  }
  if (/repossession|foreclosure|profit and loss/.test(`${status} ${paymentStatus}`)) {
  ...
  if (/\b(30|60|90|120)\s*days?\s*late\b/.test(paymentHistory) ||
      /past due/.test(paymentStatus)) {
...
function classifyRemark(item) {
  if (NEGATIVE_MARKER_RE.test(item.span?.text || '')) {
