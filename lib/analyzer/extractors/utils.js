 ...
 * derogatory path.
 *
 * All patterns are case-insensitive and use \s* between required words so that varied
...
export function detectFraudMarkers(text) {
  ...
  for (const lineText of lines) {
    ...
    const normalizedLine = normalizeMarkerLine(lineText);
    if (normalizedLine && !isLineNegated(normalizedLine)) {
      for (const { type, re, confidence } of FRAUD_MARKER_PATTERNS) {
        const match = normalizedLine.match(re);
        if (!match) continue;
        ...
        seenSpans.add(key);
        const ctxStart = Math.max(0, lineStart - 60);
        const ctxEnd = Math.min(text.length, lineEnd + 60);
        out.push({
          type,
