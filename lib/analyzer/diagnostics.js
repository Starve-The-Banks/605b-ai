...
export function reduceStatus({ findings, reviewOnly, suspectedUncertain, extracted }) {
  ...
  // as anything but findings, but if they somehow did, we still refuse to
  // call the report 'clean'. Operational markers force at least 'review_only'.
  if (hasOperational || extracted.collections.length > 0 || extracted.publicRecords.length > 0) {
    return {
      reportStatus: REPORT_STATUS_V1.REVIEW_ONLY,
