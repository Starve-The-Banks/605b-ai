...
Core standard:
- A clean report is a valid successful outcome ONLY when the extracted report text is sufficient, readable, and actually reviewed.
- Do not return a clean/no-issues result for empty, garbled, truncated, or insufficient report text.
- It is acceptable to return zero findings only when the evidence supports a clean report.
...
- If extracted PDF text is weak or garbled, do not call it clean. Return zero findings only with a low-confidence review status and set summary.overallAssessment to "${LOW_QUALITY_TEXT_MESSAGE}"
