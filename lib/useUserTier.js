...
export function useUserTier() {
  ...
  const syncSession = useCallback(async (sessionId) => {
    ...
    try {
      const res = await fetchWithTimeout('/api/stripe/sync-session', {
        method: 'POST',
  ...
  const fetchServerTierData = useCallback(async () => {
    ...
    try {
      ...
      if (err?.name === 'AbortError') {
        console.warn('[useUserTier] Tier fetch timed out — using offline fallback');
      } else {
  ...
  const loadTierData = useCallback(async (forceServerFetch = false) => {
    ...
    try {
      ...
      console.error('[useUserTier] loadTierData error:', e);
    } finally {
      setLoading(false);
    }
