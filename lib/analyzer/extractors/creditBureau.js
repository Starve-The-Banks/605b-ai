...
const COLLECTION_ACCOUNT_RE = /\b(collections?|collection account)\b/i;
const STRONG_COLLECTION_RE = /\b(collection account|in collection|placed for collection|assigned to collection|sold to (?:a )?collection|collection agency)\b/i;
const CHARGE_OFF_RE = /\b(charge[_-\s]?off|charged\s*off|charge[-\s]?off sold)\b/i;
const DEBT_BUYER_RE = /\bdebt buyer\b/i;
const NORMALIZATION_FIELD_LABELS = [
  'Original Creditor',
  'Account Name',
  'Account Type',
  'Account Number',
...
export function pushTradeline({ blockText, span, accountName, accounts, collections, defaultBureau = '' }) {
  const normalizedEvidence = normalizeEvidenceSnippet(blockText);
  const normalizedFields = parseNormalizedFields(normalizedEvidence);
  const fields = {
    ...normalizedFields,
    ...parseKeyValueLines(blockText),
  };
  ...
  const resolvedAccountName = resolveAccountName(accountName, fields, normalizedEvidence);
  const confidenceSignals = [resolvedAccountName, status || paymentStatus, balance || pastDue || paymentHistory || remarks]
  ...
  const statusSignalText = [
    status,
    paymentStatus,
    getField(fields, 'Account Condition', 'Account Status', 'Status'),
  ].filter(Boolean).join(' ');
  const remarkSignalText = [
    remarks,
    getField(fields, 'Comments', 'Remark', 'Remarks'),
  ].filter(Boolean).join(' ');
  const explicitCollectionSignal = STRONG_COLLECTION_RE.test(`${statusSignalText} ${remarkSignalText}`);
  const debtBuyerWithSupport =
    DEBT_BUYER_RE.test(String(accountType || '')) &&
    (explicitCollectionSignal ||
      CHARGE_OFF_RE.test(`${statusSignalText} ${remarkSignalText}`) ||
      /\b(past due|delinquent)\b/i.test(statusSignalText));
  const isCollectionAccount =
    (/\bcollection\b/i.test(statusSignalText) && !/\bcollections?\s*:\s*none\b/i.test(statusSignalText)) ||
    explicitCollectionSignal ||
    debtBuyerWithSupport ||
    (COLLECTION_ACCOUNT_RE.test(String(accountType || '')) &&
      /\bcollection\b/i.test(String(statusSignalText || '')));
  ...
  if (isCollectionAccount) {
    collections.push({
      ...
      bureau,
      source: bureau,
      furnisher: originalCreditor || resolvedAccountName,
      ...
      fields,
      ...normalized,
      sourceSpanNormalized: normalizedEvidence,
    });
  } else {
    accounts.push({
...
function normalizeEvidenceSnippet(raw) {
  let text = String(raw || '');
  text = text
    .replace(/\r/g, '\n')
  ...
  for (const label of NORMALIZATION_FIELD_LABELS) {
    const escaped = escapeRegExp(label);
    const hasColon = new RegExp(`\\b${escaped}\\s*:`, 'i');
    if (hasColon.test(text)) {
      text = text.replace(new RegExp(`\\b${escaped}\\s*:`, 'ig'), `\n${label}: `);
    } else {
      text = text.replace(new RegExp(`\\b${escaped}\\s*`, 'ig'), `\n${label}: `);
    }
  }
...
function parseNormalizedFields(normalizedText) {
  const out = {};
  const keyMap = {
  ...
  for (const [label, key] of Object.entries(keyMap)) {
    const value = extractLabelValue(normalizedText, label);
    if (!value) continue;
    if (key === 'Balance') {
      const money = parseCurrencyString(value);
      out[key] = money.raw || value;
      continue;
    }
    out[key] = value;
  }
...
function extractLabelValue(normalizedText, label) {
...
function resolveAccountName(accountName, fields, normalizedEvidence) {
  const candidate = [
    accountName,
    fields['Original Creditor'],
    fields['Account Name'],
    fields.Creditor,
    fields.Furnisher,
    inferNameFromEvidence(normalizedEvidence),
  ].find((value) => isHumanName(value));
  return String(candidate || '').trim();
}

function inferNameFromEvidence(text) {
  const match = String(text || '').match(
    /\b(?:Original Creditor|Creditor|Furnisher|Account Name)\s*:\s*([A-Za-z0-9&.'/ -]{3,80})/i,
  );
  return match?.[1]?.trim() || '';
}
...
function isHumanName(value) {
  const raw = String(value || '').trim();
  if (!raw) return false;
  if (/^Type[A-Z]/.test(raw)) return false;
  if (/^(Account Type|Status|Balance|Date Opened|Responsibility)$/i.test(raw)) return false;
