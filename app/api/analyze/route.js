...
/**
 * Shared entry point for the analysis pipeline. Called by:
 *   - POST /api/analyze  (direct upload)
 *   - POST /api/analyze/from-upload  (chunked upload, after reconstruction)
 *
...
export async function runAnalysisPipeline(buffer, filename, userId, opts = {}) {
  ...
  try {
    ...
    try {
      ...
      console.warn('[Analyze] Tier pre-check failed; continuing:', tierErr?.message || tierErr);
    }

    // ============================================================
    // 3. VALIDATE BUFFER (size + magic bytes)
    // ============================================================
    if (buffer.length > MAX_FILE_SIZE) {
      console.warn('[Analyze] Buffer size exceeds limit:', buffer.length);
      return errorResponse('FILE_TOO_LARGE', 'File must be under 4MB', 413);
    }

    const pdfMagic = buffer.slice(0, 4).toString('ascii');
    if (pdfMagic !== '%PDF') {
      console.warn('[Analyze] Magic byte check failed:', JSON.stringify(pdfMagic));
    ...
    let pdfData;
    try {
      console.log('[Analyze] Starting PDF parsing...', { byteSize: buffer.length });
    ...
    if (extractedText.length < 50) {
      console.warn('[Analyze] PDF text too short:', { textLength: extractedText.length, pages: pdfData.numpages });
    ...
    if (wordsFound < 10) {
      console.warn('[Analyze] PDF appears image-only:', { textLength: extractedText.length, wordsFound });
    ...
    if (!textQuality.analyzable) {
      console.warn('[Analyze] PDF text quality too weak:', { reason: textQuality.reason, textLength: extractedText.length });
    ...
    try {
      ...
      const textToAnalyze = extractedText.length > maxTextLength
        ? (() => {
            console.warn('[Analyze] Text truncated:', { originalLength: extractedText.length, truncatedLength: maxTextLength });
      ...
      analysisResult.summary = { ...(analysisResult.summary || {}), ...(fastPath ? { fastPath: true } : {}) };
      console.log('[ANALYZE TIME]', { ms: elapsed, fastPath, pages: pdfData.numpages, issues: analysisResult?.findings?.length ?? 0 });
    } catch (err) {
    ...
    try {
      ...
      if (tierRaw) {
        ...
        console.warn('[Analyze] No tier data in Redis; seeding:', userId);
      }
      ...
      console.log('[Analyze] Updated usage:', { userId, oldUsed: currentUsed, newUsed, maxAnalyses, remaining });
    } catch (usageErr) {
    ...
    try {
      ...
      console.log('[Analyze] Saved analysis record:', { userId, analysisId: savedAnalysis.id, reportType: savedAnalysis.reportType });
    } catch (storeErr) {
    ...
    return successResponse({
      filesProcessed: [{ name: filename || 'report.pdf', pages: pdfData.numpages || 1 }],
    ...
    Sentry.captureException(err, { tags: { route: 'api/analyze', stage: 'pipeline_unhandled' } });
    console.error('[Analyze] UNHANDLED ERROR in runAnalysisPipeline:', err?.stack || err);
