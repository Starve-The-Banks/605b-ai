...
export default function TrackerPage() {
  ...
  useEffect(() => {
    ...
    const checkDeadlines = () => {
      ...
      // Store notifications for other pages to access
      localStorage.setItem('605b_notifications', JSON.stringify(newNotifications));

      // Trigger email notifications if enabled and there are urgent items
  ...
  const triggerEmailNotification = async (urgentNotifications) => {
    // Check if we've already sent email today to avoid spam
    ...
    <>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: '12px' }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              {notifications.slice(0, 3).map((notif, i) => (
                <div key={notif.id} style={{ fontSize: '13px', color: '#a1a1aa', marginBottom: '4px' }}>
                  • {notif.message}
                </div>
              ))}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px', marginBottom: '24px' }}>
        {[
          ...
          { label: 'Pending Response', value: stats.pendingResponse, icon: Clock, color: '#3b82f6' },
