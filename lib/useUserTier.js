...
const PAYMENT_PENDING_TTL_MS = 15 * 60 * 1000; // 15 minutes max for pending state
...
export function useUserTier() {
  ...
  const pollingStartedRef = useRef(false); // Prevent duplicate polling starts
  ...
  const checkForPendingPayment = useCallback(() => {
    ...
    const pendingPaymentRaw = localStorage.getItem(PAYMENT_POLL_STORAGE_KEY);
    ...
    if (pendingPaymentRaw) {
      try {
        ...
        // TTL check: Clear stale pending flags (older than 15 minutes)
        if (pendingPayment.startedAt) {
          const startedAt = new Date(pendingPayment.startedAt).getTime();
          const now = Date.now();
          if (now - startedAt > PAYMENT_PENDING_TTL_MS) {
            console.log('[useUserTier] Clearing stale pending payment flag (TTL expired)');
            localStorage.removeItem(PAYMENT_POLL_STORAGE_KEY);
            pendingPayment = null;
          }
        }
      } catch (e) {
        console.error('[useUserTier] Error parsing pending payment:', e);
        localStorage.removeItem(PAYMENT_POLL_STORAGE_KEY);
      }
    }
    ...
    if (isPaymentSuccess || pendingPayment) {
      return {
        isPending: true,
        expectedTier: purchasedTier || pendingPayment?.tier || null,
        expectedAddon: purchasedAddon || pendingPayment?.addon || null,
        sessionId: sessionId || pendingPayment?.sessionId || null,
      };
    }
  ...
  const startPaymentPolling = useCallback(async (expectedTier, sessionId = null) => {
    // CRITICAL: Prevent duplicate polling starts (React StrictMode, re-renders)
    if (pollingStartedRef.current) {
      console.log('[useUserTier] Polling already started, skipping duplicate');
      return;
    }
    ...
    const poll = async () => {
      ...
      if (pollAttemptsRef.current >= PAYMENT_POLL_MAX_ATTEMPTS) {
        console.warn('[useUserTier] Payment polling timeout - clearing pending flag');
        if (pollIntervalRef.current) {
          clearInterval(pollIntervalRef.current);
          pollIntervalRef.current = null;
        }
        pollingStartedRef.current = false;
        setIsPollingForPayment(false);
        // CRITICAL FIX: Clear the pending flag to prevent infinite restart loop
        localStorage.removeItem(PAYMENT_POLL_STORAGE_KEY);
  ...
  const loadTierData = useCallback(async (forceServerFetch = false) => {
    ...
    if (isSignedIn) {
      ...
      if (serverData) {
        ...
        // Do NOT set paymentSyncComplete here — that flag is only for the post-checkout
        // banner after a successful sync/poll. Setting it on every load showed "Payment
        // Confirmed" to all paid users forever (dismiss could not persist across routes).
  ...
  // Retry sync for a specific session (for "Retry" button after timeout)
  const retrySyncSession = useCallback(async (sessionId) => {
    if (!sessionId || !isSignedIn) {
      console.error('[useUserTier] Cannot retry sync: missing sessionId or not signed in');
      return false;
    }

    setError(null);
    setIsPollingForPayment(true);
    console.log('[useUserTier] Retrying sync-session for:', sessionId);

    const result = await syncSession(sessionId);
    setIsPollingForPayment(false);

    if (result) {
      console.log('[useUserTier] Retry sync-session succeeded!');
      return true;
    } else {
      console.error('[useUserTier] Retry sync-session failed');
      setError('sync_failed');
      return false;
    }
  }, [isSignedIn, syncSession]);
  ...
  return {
    ...
    startPaymentPolling,
    retrySyncSession,
    // Current session from URL (for retry button)
    currentSessionId: searchParams?.get('session_id') || null,
      ...
      <div style={{
        ...
        <div style={{ minWidth: 0 }}>
          ...
          <div style={{ fontSize: '13px', color: '#a1a1aa', lineHeight: 1.5 }}>
            ...
            Contact <a href={SUPPORT_MAILTO} style={{ color: '#ef4444' }}>{SUPPORT_EMAIL}</a> for billing/account questions.
          </div>
        </div>
      </div>
    );
      ...
      <div style={{
        ...
        <div style={{ minWidth: 0 }}>
          ...
          <div style={{ fontSize: '13px', color: '#a1a1aa', lineHeight: 1.5 }}>
            ...
            Contact <a href={SUPPORT_MAILTO} style={{ color: '#fbbf24' }}>{SUPPORT_EMAIL}</a> for billing/account help.
          </div>
        </div>
      </div>
    );
