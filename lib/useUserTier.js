...
export function useUserTier() {
  ...
  const startPaymentPolling = useCallback(async (expectedTier) => {
    console.log('[useUserTier] Starting payment polling for tier:', expectedTier);
  ...
  const loadTierData = useCallback(async (forceServerFetch = false) => {
    ...
    // If payment is pending, DON'T use localStorage - wait for server
    if (!pendingPayment.isPending && !forceServerFetch) {
      // Check localStorage first for instant load (only when no pending payment)
    ...
    if (isSignedIn) {
      ...
      if (serverData) {
        ...
        // If we have pending payment and server shows paid tier, clear the flag
        if (pendingPayment.isPending && serverData.tier !== 'free') {
          console.log('[useUserTier] Payment already reflected in server data');
          localStorage.removeItem(PAYMENT_POLL_STORAGE_KEY);
          setPaymentSyncComplete(true);
        } else if (pendingPayment.isPending && serverData.tier === 'free') {
          // Payment pending but server still shows free - start polling
          if (!isPollingForPayment && pendingPayment.expectedTier) {
            startPaymentPolling(pendingPayment.expectedTier);
          }
        }
      }
    }
