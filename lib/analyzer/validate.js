...
function defaultIssueForRule(item) {
  switch (item.classifierRule) {
    ...
    case 'account.collection_strong':
      return 'Account is reporting as a collection account.';
    case 'account.collection_weak_signal':
    case 'collection.weak_signal':
      return 'Collection-like language detected; verify account status manually.';
...
function displaySubtitleForItem(item, context = accountContextForItem(item)) {
  ...
  if (isStrongCollectionItem(item)) {
...
function templateIdForItem(item, { issue = '', statute = '' } = {}) {
  ...
  if (isStrongCollectionItem(item)) return 'fdcpa-809';
...
function resolveAccountIdentity(item, context) {
  ...
  const fallbackTitle = isStrongCollectionItem(item)
    ? 'Collection account'
...
function isStrongCollectionItem(item) {
  const rule = String(item.classifierRule || '');
  return item.subtype === 'collection' ||
    rule === 'collection.is_collection' ||
    rule === 'account.collection_strong';
}
