...
const STRONG_COLLECTION_RE = /\b(collection account|in collection|collection agency|assigned to collection|placed for collection|sold to (?:a )?collection|collection\/chargeoff)\b/i;
const WEAK_COLLECTION_RE = /\b(collections?|debt buyer)\b/i;
const CHARGE_OFF_RE = /\b(charge[_-\s]?off|charged\s*off|charge[-\s]?off sold)\b/i;
...
function classifyAccount(item) {
  ...
  const accountCondition = (item.fields?.['Account Condition'] || item.fields?.['Account Status'] || '').toLowerCase();
  const accountType = (item.accountType || item.loanType || item.fields?.['Account Type'] || item.fields?.Type || '').toLowerCase();
  const sourceText = normalizeSourceText(item.span?.text || '');
  const explicitStatusText = `${status} ${paymentStatus} ${remarks} ${accountCondition}`.trim();
  const combinedStatus = `${explicitStatusText} ${accountType}`.trim();

  if (hasStrongCollectionEvidence({ explicitStatusText, accountType, sourceText })) {
    return withClassification(item, CLASSIFICATIONS.HIGH_PRIORITY, 'account.collection_strong',
      'Account status or remarks indicate collection reporting.', 0.95, { subtype: 'collection' });
  }
  if (hasWeakCollectionEvidence({ explicitStatusText, accountType, sourceText })) {
    return withClassification(item, CLASSIFICATIONS.REVIEW_ONLY, 'account.collection_weak_signal',
      'Collection-like language is ambiguous; manual review only.', 0.62, { subtype: 'collection_review' });
  }
  if (CHARGE_OFF_RE.test(explicitStatusText)) {
...
function classifyCollection(item) {
  ...
  const accountType = String(item.accountType || item.loanType || item.fields?.['Account Type'] || item.fields?.Type || '').toLowerCase();
  const accountCondition = String(item.fields?.['Account Condition'] || item.fields?.['Account Status'] || '').toLowerCase();
  const explicitStatusText = `${status} ${item.paymentStatus || ''} ${item.remarks || ''} ${accountCondition}`.toLowerCase();
  const sourceText = normalizeSourceText(item.span?.text || '');
  if (!hasStrongCollectionEvidence({ explicitStatusText, accountType, sourceText })) {
    if (hasWeakCollectionEvidence({ explicitStatusText, accountType, sourceText })) {
      return withClassification(item, CLASSIFICATIONS.REVIEW_ONLY, 'collection.weak_signal',
        'Collection bucket lacks strong collection evidence; manual review only.', 0.62, { subtype: 'collection_review' });
    }
...
function normalizeSourceText(value) {
  return String(value || '')
    .toLowerCase()
    .replace(/\bcollections?\s*:\s*none\b/g, ' ')
    .replace(/\bokcurrent3030\s*days?\s*late.*?ngnegative\s*nanot\s*available\b/g, ' ')
    .replace(/\s+/g, ' ')
    .trim();
}

function hasStrongCollectionEvidence({ explicitStatusText, accountType, sourceText }) {
  const statusText = String(explicitStatusText || '');
  const typeText = String(accountType || '');
  const source = String(sourceText || '');
  if (STRONG_COLLECTION_RE.test(statusText)) return true;
  if (/\bcollection\b/.test(statusText) && !/\bcollections?\s*:\s*none\b/.test(statusText)) return true;
  if (CHARGE_OFF_RE.test(statusText) && /\b(collection|collection agency|debt buyer)\b/.test(statusText)) return true;
  if (/\bdebt buyer\b/.test(typeText) && (STRONG_COLLECTION_RE.test(statusText) || CHARGE_OFF_RE.test(statusText) || /\b(past due|delinquent)\b/.test(statusText))) {
    return true;
  }
  if (/\b(derogatory information|collections?)\b/.test(source) && STRONG_COLLECTION_RE.test(statusText)) return true;
  return false;
}

function hasWeakCollectionEvidence({ explicitStatusText, accountType, sourceText }) {
  const statusText = String(explicitStatusText || '');
  const typeText = String(accountType || '');
  const source = String(sourceText || '');
  if (STRONG_COLLECTION_RE.test(statusText)) return false;
  return WEAK_COLLECTION_RE.test(`${statusText} ${typeText} ${source}`);
}
