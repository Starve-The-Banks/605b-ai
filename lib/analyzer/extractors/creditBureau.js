...
export function extractCreditBureauReport(text, ctx) {
  ...
  for (const section of sections) {
    ...
    } else if (/^fraud alerts/.test(labelLower) || /^consumer (statements|disputes)/.test(labelLower)) {
      const body = section.text.trim();
      if (body && !isNoneMarker(body) && hasNegativeMarker(body)) {
        const span = makeSpan(text, section.start, section.end);
        fraudMarkers.push({
