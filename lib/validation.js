...
// Email notification schema — recipient is resolved server-side from Clerk; not accepted from client.
...
// intentId: client-generated UUID, stable across retries for the same purchase
// intent. Used server-side as the Stripe idempotency key so repeat clicks /
// network retries return the ORIGINAL checkout session instead of duplicating.
const UUID_V4_RE =
...
export const stripeCheckoutSchema = z.object({
  ...
  intentId: z
    .string()
    .regex(UUID_V4_RE, { message: 'intentId must be a v1-5 UUID' })
    .optional(),
}).refine(
