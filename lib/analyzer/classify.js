...
const COLLECTION_INDICATOR_RE = /\b(collections?|collection account|collection agency|debt buyer|assigned to collection|placed for collection|charge[-\s]?off sold|collection\/chargeoff)\b/i;
...
function classifyAccount(item) {
  ...
  const sourceText = (item.span?.text || '')
    .toLowerCase()
    .replace(/\bcollections?\s*:\s*none\b/g, ' ');
  const combinedStatus = `${status} ${paymentStatus} ${remarks} ${accountType}`;
  const collectionSignalText = `${combinedStatus} ${sourceText}`;

  if (COLLECTION_INDICATOR_RE.test(collectionSignalText)) {
...
function classifyCollection(item) {
  ...
  const text = `${status} ${item.paymentStatus || ''} ${item.remarks || ''} ${accountType} ${item.span?.text || ''}`.toLowerCase();
  if (!COLLECTION_INDICATOR_RE.test(text)) {
