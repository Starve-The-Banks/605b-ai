...
export function useUserTier() {
  ...
  const hasLoadedRef = useRef(false); // Prevent re-fetching on every render
  ...
  const loadTierData = useCallback(async (forceServerFetch = false) => {
    // Prevent re-running if already loaded (unless forced)
    // This stops the infinite loop when state changes cause callback recreation
    if (hasLoadedRef.current && !forceServerFetch) {
      return;
    }
    ...
    if (isSignedIn) {
      ...
      if (serverData) {
        ...
        if (serverData.tier !== 'free') {
          ...
          // Use ref to check polling state to avoid dependency on isPollingForPayment
          if (!pollingStartedRef.current && pendingPayment.expectedTier) {
    ...
    setLoading(false);
  }, [isSignedIn, fetchServerTierData, checkForPendingPayment, startPaymentPolling]);
  ...
  const forceRefresh = useCallback(async () => {
    setLoading(true);
    hasLoadedRef.current = false; // Reset so loadTierData will fetch again
